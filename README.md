...
## Вывод
В ходе работы изучено взаимодействие Docker-контейнеров в общей сети. Реализована схема из двух контейнеров (nginx и php-fpm), объединённых в пользовательскую сеть internal, что позволило организовать обмен запросами между сервисами без использования docker-compose. Настроена передача PHP-запросов от nginx к backend-контейнеру через FastCGI, а также подключение общего тома с веб-файлами для обеспечения единого источника данных. В результате получен рабочий веб-сайт, доступный через браузер по адресу http://localhost.
